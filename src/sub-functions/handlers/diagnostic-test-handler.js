...
export class DiagnosticTestHandler {
  ...
  initializeState() {
    ...
    if (exerciseState && exerciseState.version === 'diagnostic-1.2') {
      ...
      this.activeTab.exerciseState = {
        ...
        answers: this.pageData.blocks.map((block) => block.exercises.map(() => ({
          userAnswer: null,
          isCorrect: null,
          notes: '',
  ...
  renderGlobalControls(parentElement) {
    ...
    buttonGroups.forEach((group) => {
      ...
      const titleExtraStyle = (group.title === 'Revert') ? 'style="background-color: #F97316;"' : '';
      ...
      const buttonsHTML = this.pageData.blocks.map((block, index) => {
        ...
        const buttonExtraStyle = (group.title === 'Revert') ? 'style="background-color: #F97316;"' : '';
  ...
  renderMultipleChoiceQuestion(question, answerState, blockSubmitted) {
    const { isComplete } = this.activeTab.exerciseState;
  ...
  renderParagraphInputQuestion(question, answerState, blockSubmitted) {
    ...
    return `
            ...
            <div class="word-bank flex flex-wrap gap-2 p-2 bg-indigo-50 rounded-md my-4">
                ${question.wordBank.correct.concat(question.wordBank.intruders).map((word) => `<span class="bg-white border border-slate-300 rounded px-2 py-1 text-sm font-medium">${word}</span>`).join('')}
            </div>
